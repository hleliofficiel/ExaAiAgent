...
def create_agent(
    ...
    try:
        ...
        if module_list:
            from exaaiagnt.prompts import get_all_module_names, validate_module_names
        ...
        from exaaiagnt.agents import ExaaiAgent
...
{recommendations_xml}
                ...
                if (
                    parent_state
                    and hasattr(parent_state, "is_waiting_for_input")
                    and parent_state.is_waiting_for_input()
                ):
...
def send_user_message_to_agent(agent_id: str, message: str) -> dict[str, Any]:
    try:
        ...
        if (
            target_state
            and hasattr(target_state, "is_waiting_for_input")
            and target_state.is_waiting_for_input()
        ):
