...
from exaaiagnt.interface.utils import build_live_stats_text
...
class SplashScreen(Static):  # type: ignore[misc]
    ...
    # Enhanced ASCII Logo - ExaAi v2.2.6
    BANNER = r"""
    ███████╗██╗  ██╗ █████╗      █████╗ ██╗
    ██╔════╝╚██╗██╔╝██╔══██╗    ██╔══██╗██║
    █████╗   ╚███╔╝ ███████║    ███████║██║
   ...
   ░▒████▒▒██▒ ▒██▒ ▓█   ▓██▒     ▓█   ▓██▒░██░
    ...
    def _build_new_features_text(self) -> Text:
        """Build new features highlight for v2.2.6."""
...
class ExaaiTUIApp(App):  # type: ignore[misc]
    ...
    def _render_tool_content_simple(self, tool_data: dict[str, Any]) -> str:
        ...
        from exaaiagnt.interface.tool_components.registry import get_tool_renderer
    ...
    def action_confirm_stop_agent(self, agent_id: str) -> None:
        ...
        try:
            from exaaiagnt.tools.agents_graph.agents_graph_actions import stop_agent
